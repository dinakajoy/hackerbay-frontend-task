...
const Board = (props) => {
  ...
  const play = (e) => {
    if (weaponsDestroyed === boardWidth) {
      ...
      alert(
        `Congratulations! You defeated ${weaponsDestroyed} warriors to save the princess`
      );
    ...
    for (let column = 0; column < boardWidth; column++) {
      if (row === gridMiddle && column === gridMiddle) {
        ...
        grid[row].push(
          ...
          >
            <img src={player} alt="sprite-player" width="19px" height="19px" />
          </div>
        );
      } else if (
        numOfWeapons !== boardWidth &&
        (Math.floor(Math.random() * (1 - boardSize)) + boardSize) %
          gridMiddle ===
          boardWidth % 5
      ) {
        ...
        grid[row].push(
          <div
            key={`x:${row}, y:${column}`}
            id={`x:${row}, y:${column}`}
            style={{ width: "25px", height: "25px", border: "1px solid #333" }}
          >
  ...
  return (
    <>
      <input
        ...
        name="custId"
        onChange={handleMob}
        style={{ height: 0, border: 0, outline: 0 }}
      />
      <div
        style={{
          display: "grid",
